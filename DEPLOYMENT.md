...
- **At least one AI API Key:**
  - **Claude API Key** from [Anthropic Console](https://console.anthropic.com/) (recommended)
...
CLAUDE_API_KEY=sk-ant-api03-your-actual-key      # Required if using Claude
GEMINI_API_KEY=your-gemini-api-key               # Required if using Gemini

# Authentication
JWT_SECRET=generate-a-random-32-char-string      # Required (use: openssl rand -base64 32)

# Database
POSTGRES_PASSWORD=your-secure-password           # Required
DATABASE_URL=postgresql://takoss:${POSTGRES_PASSWORD}@localhost:5432/takoss_db
...
**AI Model Selection:**
- Users can choose between Claude and Gemini when creating projects
- You only need the API key for the model(s) you want to support
- Claude: Better code quality, requires paid credits
- Gemini: Faster and more cost-effective, has free tier
...
- `POST /api/generate` - Generate application (requires auth)
  ...
  **Model Selection Options:**
  - `"claude"` - Uses Claude Sonnet 4.5 (best quality, requires CLAUDE_API_KEY)
  - `"gemini"` - Uses Gemini 2.0 Flash (faster, requires GEMINI_API_KEY)
  - If omitted, defaults to Claude

  **Generation Phases (6 total):**
  1. Requirements Analysis - Extracts entities, features, relationships
  2. Complexity Estimation - Calculates project complexity score
  3. Database Schema - Generates Prisma schema
  4. Frontend Components - Creates React + TypeScript components
  5. Backend API - Generates Express routes with Prisma integration
  6. Deployment Config - Creates Docker files and CI/CD configs
