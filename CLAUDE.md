...
CLAUDE_API_KEY=sk-ant-api03-...     # Required if using Claude - from console.anthropic.com
GEMINI_API_KEY=...                  # Required if using Gemini - from makersuite.google.com
...
- **Orchestrator** (`src/orchestrator/simpleTakossOrchestrator.ts`) - Main generation pipeline with model selection
- **Model Support** (`src/core/modelService.ts`) - Supports Claude Sonnet 4.5 and Gemini 2.0 Flash
...
- **Model Selection UI** - Choose between Claude or Gemini when creating projects
...
- Main generation pipeline orchestrator with **multi-model support**
- Dynamically creates decomposer instances with selected AI model (Claude or Gemini)
- Executes 6 phases in sequence with AI-powered code generation:
  1. **Requirements Analysis** - Extracts entities, features, relationships, and UI requirements (supports Gemini)
  2. **Complexity Estimation** - Rule-based scoring to determine project complexity
  3. **Database Schema** - Generates complete Prisma schema from entities with AI refinement (supports Gemini)
  4. **Frontend Components** - Generates React/TypeScript components for ALL UI requirements (supports Gemini)
  5. **Backend API** - Generates Express routes with CRUD operations, validation, and Prisma integration (supports Gemini)
  6. **Deployment Configuration** - Creates Dockerfile, docker-compose.yml, and CI/CD configs (supports Gemini)
...
**Model Selection Architecture** (`src/core/modelService.ts` + decomposers)
- **ModelService**: Unified interface for Claude and Gemini APIs
- **Claude Integration**: Uses LangChain's ChatAnthropic for Claude Sonnet 4.5
- **Gemini Integration**: Uses Google Generative AI SDK for Gemini 2.0 Flash
- **Smart Routing**: Each decomposer detects selected model and routes to appropriate API
- **Decomposers Updated**: RequirementsAnalyzer, ComponentDecomposer, DatabaseSchemaEvolution, APIEndpointDecomposer, DeploymentTaskDecomposer
...
2. **Create Project Request** → `POST /api/generate` with auth header and `selectedModel` parameter
3. **Model Validation** → Backend checks if required API key (Claude or Gemini) is configured
4. **Orchestrator Execution** → 6 phases executed sequentially with selected AI model:
...
5. **File Generation** → ProjectWriter creates complete project structure from generated code
6. **ZIP Creation** → Archive created for download
7. **Response** → Project metadata returned with success status
8. **View/Download** → User can browse files or download ZIP

**Model Selection Flow:**
- Frontend: User selects "Claude" or "Gemini" via UI button
- API Layer: `selectedModel` parameter sent to `/api/generate`
- Backend: Validates API key availability, maps to model config
- Orchestrator: Creates decomposer instances with selected model
- Decomposers: Route to ModelService (Gemini) or LangChain (Claude)
- Generation: All 6 phases execute with chosen AI model
