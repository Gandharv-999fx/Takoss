...
export class RequirementsAnalyzer {
  ...
  private selectedModel?: ModelName;
  private useModelService: boolean = false;

  constructor(apiKey?: string, modelName?: ModelName) {
    // If a Gemini model is specified, use ModelService instead of LangChain
    ...
    this.model = new ChatAnthropic({
      ...
      modelName: modelName && modelName.includes('claude') ? modelName as any : 'claude-sonnet-4-5-20250929',
...
`);
    ...
    // Use ModelService for Gemini, LangChain for Claude
    if (this.useModelService && this.modelService && this.selectedModel) {
      console.log(`  Using Gemini model: ${this.selectedModel}`);
      const geminiResponse = await this.modelService.executePrompt(input, 'gemini', {
        temperature: 0.3,
        maxTokens: 8192,
      });
      content = geminiResponse.content;
    } else {
      console.log(`  Using Claude model via LangChain`);
