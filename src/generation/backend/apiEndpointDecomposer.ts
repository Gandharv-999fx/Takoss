...
`);
    ...
    if (this.useModelService && this.modelService) {
      console.log(`      Decomposing feature with Gemini: ${feature.name}`);
...
`;
  ...
  public async generateEndpointCode(
    ...
    // Use ModelService for Gemini, LangChain for Claude
    if (this.useModelService && this.modelService) {
      console.log(`      Generating endpoint with Gemini: ${endpointPrompt.endpointDef.path}`);
      const geminiResponse = await this.modelService.executePrompt(endpointPrompt.prompt, 'gemini', {
        temperature: 0.3,
        maxTokens: 8192,
      });
      content = geminiResponse.content;
