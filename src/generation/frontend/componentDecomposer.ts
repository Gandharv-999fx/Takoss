...
export class ComponentDecomposer {
  ...
  constructor(apiKey?: string, modelName?: string) {
    ...
    if (modelName && modelName.includes('gemini')) {
      ...
      this.selectedModel = modelName as ModelName;
...
`);
    ...
    if (this.useModelService && this.modelService) {
      console.log(`    Using Gemini for component decomposition`);
...
`;
  ...
  public async generateComponent(componentPrompt: ComponentPrompt): Promise<string> {
    ...
    // Use ModelService for Gemini, LangChain for Claude
    if (this.useModelService && this.modelService) {
      console.log(`    Generating ${componentPrompt.componentName} with Gemini`);
      const geminiResponse = await this.modelService.executePrompt(componentPrompt.prompt, 'gemini', {
        temperature: 0.3,
        maxTokens: 8192,
      });
      content = geminiResponse.content;
